...
session_start();
$id= $_SESSION['UID'];
$queryuser=mysqli_query($conn, "SELECT * FROM tb_user WHERE id='$id'");
$user=mysqli_fetch_assoc($queryuser);
$jumlahDataPerHalaman = 6;
$jumlahData =count(query("SELECT * FROM tb_data_keuangan"));
...
//data siswa
    $data = query("SELECT * FROM tb_data_keuangan LIMIT $awalData,$jumlahDataPerHalaman");
if( isset($_POST["cari"] )) {
    $data = caridatakeuangan($_POST['keyword_jadwal']);
}
if(!isset($_SERVER["PATH_INFO"])=="dashboard-keuangan.php"){
 header("location: dashboard-keuangan");
 exit;
}
        ...
        <div class="w-100">
          <nav class="navbar p-10 navbar-dark">
            <div class="container-fluid">
              ...
              <div class="d-flex">
                <div class="profile " style="width:30px; height:30px; border-radius:50%; margin-top:auto; margin-bottom:auto; background-position:center; background-repeat:no-repeat; background-size:cover; background-image:url('foto/<?= $user["gambar"];?>');"></div>
                <a href="#" class="nav-link text-white ms-2 me-2 mt-auto mb-auto"> <?= $user["namalengkap"];?></a>
               <ul class="navbar-nav">
          ...
          <div class="content">
            <div class="container-fluid">
              <div class="title">
                <h1 class="text-white ms-3 mt-3 mb-3">Dashboard keuangan</h1>
              </div>
              <div class="row mt-5 ms-lg-3 me-lg-3">
                <div class="container">
                  <a class="btn btn-success col-lg-2" href="data-keuangan-action/add.php"
                    >+ Tambah Data Kas</a
                  >
                </div>
              </div>
                ...
                <table class="table text-white">
                    ...
                    <thead>
                      <tr>
                        ...
                        <th scope="col">id_absen</th>
                        <th scope="col">TOTAL UANG MURID</th>
                    ...
                    foreach($data as $rows):
                        ...
                        $id_users=$rows["id_user"];
                        $query_users=mysqli_query($conn, "SELECT * FROM  tb_user WHERE id=$id_users");
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td><?= $rows["jumlah_uang"];?></td>
                        <td><?= $rows["date"];?></td>
                        <td><?= $rows["nomor_absen"];?></td>
                        <td><?= $rows["total_uang"];?></td>
                        <td>
                           
                          <a href="data-keuangan-action/edit.php?id=<?= $rows["id"];?>">edit</a>
                        <a href="data-keuangan-action/delete.php?id=<?= $rows["id"];?>" onclick="return confirm('yakin?');">delete</a>
                        </td>
                   ...
                   <?php
                    if(!$data){
                      echo "<p class='text-danger text-center'>Empty</p>";
                    }
                    ?>
